...
export default function PwaInstallBanner() {
  ...
  useEffect(() => {
    // 1. Vérifier si l'application est déjà lancée en mode autonome (PWA installée)
    const isStandalone =
    ...
    // 3. Détecter si l'appareil est sous iOS (Safari iPhone/iPad)
    ...
    // 4. Capturer l'événement de téléchargement Chrome / Android / Desktop
  ...
  return (
    <>
      {/* Banner Call-To-Action sticky en bas de page sur Mobile & Desktop */}
      <div className="fixed bottom-3 left-3 right-3 sm:left-auto sm:right-6 sm:max-w-md z-40 animate-fade-in-up">
        <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-indigo-500/30 text-white rounded-2xl p-4 shadow-2xl backdrop-blur-xl flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-11 h-11 rounded-xl bg-indigo-600/30 border border-indigo-400/30 flex items-center justify-center shrink-0 text-indigo-400">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white transition-all shadow-md shadow-indigo-600/30 cursor-pointer"
            >
      ...
      {/* Modal d'instructions d'installation pour iOS (Safari iPhone) */}
      {showIosModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl space-y-5 animate-scale-up">
            ...
            <button
              onClick={() => setShowIosModal(false)}
              className="w-full py-2.5 rounded-xl text-xs font-bold bg-indigo-600 text-white hover:bg-indigo-500 transition-colors"
            >
