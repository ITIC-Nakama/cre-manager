    ...
    <>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white">Étudiants à suivre</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              {loading ? '…' : `${students.length} étudiant${students.length > 1 ? 's' : ''} · top 5 prioritaires`}
            </p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="text-center py-10 text-sm text-slate-400">Aucun étudiant enregistré.</div>
        ) : (
        ...
        {!loading && !hasAlerts && students.length > 0 && (
          <div className="px-6 py-3 border-t border-slate-100 dark:border-slate-800 text-xs text-emerald-600 dark:text-emerald-400 text-center">
            Tous les étudiants sont à jour — aucune alerte active.
          </div>
        )}
      </div>
