...
import { ArrowLeft, Loader2, UserMinus, UserPlus, Search, GraduationCap, ShieldCheck, ShieldAlert } from 'lucide-react';
...
export default function PromotionDetailPage() {
  ...
  const { data, isLoading: loadingStudents } = useStudentList({ promotionId: id, size: 500 });
  ...
  }, [students, selectedYearTab, filterText]);
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <span className="text-xs text-slate-400 self-end sm:self-center">
            {t('dashboard.promotions.filtered_count', { count: displayedStudents.length, defaultValue: `${displayedStudents.length} affiché(s)` })}
          </span>
        </div>
