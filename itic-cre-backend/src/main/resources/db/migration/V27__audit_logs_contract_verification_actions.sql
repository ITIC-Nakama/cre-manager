-- Ajoute les actions APPLICATION_CONTRACT_* a la contrainte CHECK de audit_logs.action (voir AuditAction.java).
-- Meme incident deja rencontre a plusieurs reprises : oublier de reconstruire cette contrainte
-- quand l'enum gagne de nouvelles valeurs fait echouer l'insertion de l'audit log des le premier
-- appel avec une nouvelle action.
...
ALTER TABLE audit_logs ADD CONSTRAINT audit_logs_action_check CHECK (action IN (
    ...
    'JOB_OFFER_WIPED',
    'APPLICATION_CONTRACT_VERIFIED','APPLICATION_CONTRACT_REJECTED',
    'OTHER'
));
