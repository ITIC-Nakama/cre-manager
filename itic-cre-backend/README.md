...
- **Conformité RGPD automatisée & Configuration Applicative BDD** : `GdprPurgeScheduler`, `GdprService` et `AppConfigurationService` assurent la gouvernance RGPD :
  - **Configuration dynamique en BDD (`app_configuration`)** : Les durées de rétention (`GDPR_OTP_RETENTION_HOURS`, `GDPR_AUDIT_LOG_RETENTION_DAYS`, `GDPR_INACTIVE_STUDENT_RETENTION_DAYS`, `GDPR_ALUMNI_RETENTION_DAYS`, `STALE_ALERT_DAYS`, `PROMOTION_REMINDER_MONTHS`) sont gérées dynamiquement par les Administrateurs via `GET/PUT /api/admin/app-config` (protégé `@PreAuthorize("hasRole('ADMIN')")`).
